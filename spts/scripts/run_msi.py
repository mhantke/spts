...
import msi.log
from msi.log import log_and_raise_error,log_warning,log_info,log_debug
logger = msi.logger

import msi.config
import msi.worker

# Add MSI stream handler to other loggers
import h5writer
#h5writer.logger.addHandler(msi._stream_handler)
import mulpro
#mulpro.logger.addHandler(msi._stream_handler)
...
if __name__ == "__main__":
    ...
    if not os.path.exists("./msi.conf"):
        parser.error("Cannot find configuration file \"msi.conf\" in current directory.")
    lvl = logging.WARNING
    if args.verbose:
        lvl = logging.INFO
    if args.debug:
        lvl = logging.DEBUG
    msi.logger.setLevel(lvl)
    h5writer.logger.setLevel(lvl)
    logging.basicConfig(level=lvl)
    
    log_info(logger, "Hostname: %s" % socket.gethostname())
        
    conf = msi.config.read_configfile("./msi.conf")
    ...
    if args.mpi:
        ...
        H = h5writer.H5WriterMPISW("./msi.cxi", comm=comm, chunksize=100, compression=None)
        if is_worker:
            W = msi.worker.Worker(conf, i0_offset=comm.rank-1, step_size=comm.size-1)
    else:
        is_worker = True
        H = h5writer.H5Writer("./msi.cxi")
        W = msi.worker.Worker(conf)
    ...
    H.write_solo({'__version__': msi.__version__})
    H.close()

    log_info(logger, "MSI - Clean exit.")
